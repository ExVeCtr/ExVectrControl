...
void ControlAttitudeFlaps::taskThread() {
  ...
  // Print the belly down angle and flap factor to the console

  ControlAttitudeFlapSetting flapSetting;
  if (controlSettings.bellyFlopMode ==
      ControlAttitudeBellyFlopSetting::BellyFlopMode::BellyFlopMode_Stabilize) {
    ...
    float azimuthOutput = controlSettings.azimuthAngle_Rad - azimuthIs;
    float pitchOutput = controlSettings.pitchAngle_Rad - pitchIs;

    if (azimuthOutput > M_PI) {
      azimuthOutput -= 2 * M_PI;
    } else if (azimuthOutput < -M_PI) {
      azimuthOutput += 2 * M_PI;
    }

    azimuthOutput = azimuthOutput * attitudeAzimuGain_;
    pitchOutput = pitchOutput * attitudePitchGain_;
    ...
    attRateCtrlOutput = attRateCtrlOutput +
                        Math::Vector<float, 3>({azimuthOutput, pitchOutput, 0});
    ...
    flapSetting.flapTLAngle_Rad =
        flapTopNeutralAngle_Rad_ +
        (-attRateCtrlOutput(0) - attRateCtrlOutput(1) + attRateCtrlOutput(2)) *
            flapFactor;
    flapSetting.flapTRAngle_Rad =
        flapTopNeutralAngle_Rad_ +
        (+attRateCtrlOutput(0) - attRateCtrlOutput(1) - attRateCtrlOutput(2)) *
            flapFactor;
    flapSetting.flapBLAngle_Rad =
        flapBottomNeutralAngle_Rad_ +
        (+attRateCtrlOutput(0) + attRateCtrlOutput(1) + attRateCtrlOutput(2)) *
            flapFactor;
    flapSetting.flapBRAngle_Rad =
        flapBottomNeutralAngle_Rad_ +
        (-attRateCtrlOutput(0) + attRateCtrlOutput(1) - attRateCtrlOutput(2)) *
            flapFactor;
