...
/**
 * @brief Subsribes to a topic to which the position estimation is published in
 * form: [V, P], where V is the linear velocity vector and P is the position
 * vector.
 */
void ControlPositionStandard::subscribePositionMeasurement(
 ...
 */
void ControlPositionStandard::subscribeSetpoint(
    Core::Topic<Math::Vector<float, 6>> &setpointTopic) {
...
void ControlPositionStandard::taskThread() {
  ...
  // setpointPosition = {0, 0, 1}; // Setpoint position in reference frame (X,
  // Y, Z) position = {1, 0, 1}; velocity = {0, 0, 0};

  // LOG_MSG("Position: %.2f %.2f %.2f\n", setpointPosition(0),
  // setpointPosition(1), setpointPosition(2));
  ...
  { // Lets keep these vars local to this scope
    auto length = sqrt(
        posCtrlOutput(0) * posCtrlOutput(0) +
        posCtrlOutput(1) *
            posCtrlOutput(1)); // Calculate the length of the horizontal vector
    if (length >
        positionHZTLimit_ms_) { // If over the limit, scale down the output
  ...
  // posCtrlOutput(1), posCtrlOutput(2)); // Print the position controller
  // output to the console

  // ################# Calculate the velocity controller output ################
  Math::Vector<float, 3> velCtrlError(
      {(posCtrlOutput(0) - velocity(0) + setpointPosition(0)),
       (posCtrlOutput(1) - velocity(1) + setpointPosition(1)),
       (posCtrlOutput(2) - velocity(2) + setpointPosition(2))});
  Math::Vector<float, 3> velCtrlOutput({velocityHZTGain_ * velCtrlError(0),
                                        velocityHZTGain_ * velCtrlError(1),
                                        velocityVRTGain_ * velCtrlError(2)});

  velCtrlIntegral_(0) =
      velCtrlIntegral_(0) + velCtrlError(0) * velocityHZTIntegral_ * dTime;
  velCtrlIntegral_(1) =
      velCtrlIntegral_(1) + velCtrlError(1) * velocityHZTIntegral_ * dTime;
  velCtrlIntegral_(2) =
      velCtrlIntegral_(2) + velCtrlError(2) * velocityVRTIntegral_ * dTime;
  ...
  // limit the output of the velocity controller including the integral. If we
  // are over the limit, with the P and I term, then we subtract from the
  // integral term so we reach the limit. But do not go over zero for the
  // integral term.
  if (velCtrlOutput(0) + velCtrlIntegral_(0) > velocityHZTLimit_mss_) {
    velCtrlIntegral_(0) = velocityHZTLimit_mss_ - velCtrlOutput(0);
    if (velCtrlIntegral_(0) < 0)
      velCtrlIntegral_(0) = 0; // Don't let the integral term go negative
  ...
  if (velCtrlOutput(1) + velCtrlIntegral_(1) > velocityHZTLimit_mss_) {
    ...
    if (velCtrlIntegral_(1) < 0)
      velCtrlIntegral_(1) = 0; // Don't let the integral term go negative
  ...
  if (velCtrlOutput(2) + velCtrlIntegral_(2) > velocityVRTLimit_mss_) {
    ...
    if (velCtrlIntegral_(2) < 0)
      velCtrlIntegral_(2) = 0; // Don't let the integral term go negative
  ...
  // velCtrlIntegral_(1), velCtrlIntegral_(2), dTime); // Print the velocity
  // controller output to the console

  // Add the gravity acceleration to be compensated to the velocity controller
  // output
  velCtrlOutput(2) += Math::GRAVITY;
  velCtrlOutput =
      velCtrlOutput +
      velCtrlIntegral_; // Add the integral to the velocity controller output

  accelTopic_.publish(
      velCtrlOutput); // Publish the velocity controller output to the topic
}
